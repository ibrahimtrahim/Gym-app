...
function SearchExercices({setExercises, bodyPart, setBodyPart}) {
  ...
  const handleSearch = async() => {
    if (search) {
      const exercesesData = await fetchData('https://exercisedb.p.rapidapi.com/exercises', exerciseOptions);

      const searchedExercises = exercesesData.filter(
  ...
  return (
    <Stack alignItems="center" mt="37px" justifyContent="center" p="20px">
      <Typography fontWeight={700} sx={{fontSize: {lg: '44px', xs:'30px'}}} mb='50px' textAlign='center'>
        Awsome Exerciscs You <br /> Should Know
      </Typography>
